...
node ('Docker') {
    ...
    stage ('Update Jarvis') {
        withDockerRegistry(credentialsId: '0435817a-5f0f-47e1-9dcc-800d85e5c335') {
            DockerJarvis=docker.image('dyalog/jarvis')
            DockerJarvis.pull()
        }
    }
    ...
    stage('Deploying with Rancher') {
        ...
        usernamePassword(credentialsId: '02543ae7-7ed9-4448-ba20-6b367d302ecc', passwordVariable: 'SECRETKEY', usernameVariable: 'ACCESSKEY')]) {
            if (env.BRANCH_NAME.contains('staging')) {
                sh '/usr/local/bin/rancher-compose -f ./docker-compose-rancher.yml --access-key $ACCESSKEY --secret-key $SECRETKEY --url https://rancher.dyalog.com/v2-beta/projects/1a5/stacks/1st41 -p TryAPL-Staging up --force-upgrade --confirm-upgrade --pull -d'
            }
            if (env.BRANCH_NAME.contains('live')) {
                sh '/usr/local/bin/rancher-compose -f ./docker-compose-rancher.yml --access-key $ACCESSKEY --secret-key $SECRETKEY --url https://rancher.dyalog.com/v2-beta/projects/1a5/stacks/1st9 -p TryAPL up --force-upgrade --confirm-upgrade --pull -d'
            }
        }
    }
}