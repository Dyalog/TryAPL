...
function loadTryAPL () {
	...
	document.addEventListener("keydown", function (event) {
		...
		if (kc === 13) {
			...
			if ( event.altKey || Continue(expr) ) {
				...
				else if (!expr.search(/^\s*\)save\s+/i)) {downloadWS();blankLine(0)}
				else if (!expr.search(/^\s*\)load\s+/i)) {$("#upload").click();blankLine(0)}
				else {
					submitLine(expr)
				}
			}
	...
	if (location.search.length > 0) {
		...
		if (["workspace", "ws"].map(x=>{if(s.get(x) != null){expr=s.get(x)}; return expr}).some(x=>x)) {
			$("#wsURL").value = expr
			wsFromURL("#wsURL")
		}
	}
