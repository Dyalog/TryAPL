...
function loadTryAPL () {
	...
	document.addEventListener("keydown", function (event) {
		...
		if (kc === 13) {
			...
			if (getSessionInput(0)[0]=="\t"||(getSessionInput(1)+"")[0]=="\t") {
			...
			if ( event.altKey || Continue(expr) ) {
				...
				if (!expr.search(/^\s*\)clear\s*/i)) {
					log("clear workspace")
					lastText=oldText=session.value += "\nclear ws\n      "
					state = ["",0,"",""]
				}
				else if (!expr.replace(/\s+⍝.*/,"").search(/^\s*\)save\s*$/i)) {downloadWS();blankLine(0)}
				else if (!expr.replace(/\s+⍝.*/,"").search(/^\s*\)load\s*$/i)) {$("#upload").click();blankLine(0)}
				else {
					submitLine(expr)
				}
			}
